...
- das UART-Interface (Baud rate 115200-8N1) kann den Bootprozess noch vor dem Laden des Linuxkernel loggen.
  ...
  - Ubuntu:  Monitoring des USB-TTL-Adapters mit `sudo dmesg -w` und `ls -l /dev/ttyUSB0`, `sudo screen -L /dev/ttyUSB0 115200 (-L -> screenlog.0)` bzw. `sudo minicom -s` und `sudo minicom -D /dev/ttyUSB0 -b 115200` ermöglicht UART-Login in den Raspberry.
  
  - `enable_uart=1` in `/boot/firmware/config.txt`, evtl. auch `uart_2ndstage=1` wie beschrieben in `http://rptl.io/configtxt` (diet-config advanced -> in cmdline.txt `console=tty1 console=serial0,115200`), dann `ls -l /dev/serial0`. Testsendung mit `echo "hello" > /dev/serial0`. Die Dateiübertragung via UART geschieht aus `minicom-kermit`, wenn auf dem Raspberry ein Kermit als Empfänger eingerichtet ist:
  ...
  - https://itsfoss.com/use-uart-raspberry-pi/ , typische Adapterchips: CH340G, CH341, CP2102, FT232,
...
- https://www.pollin.de/p/joy-it-usb-ttl-schnittstellenwandler-811109 (CH341 Chip)
