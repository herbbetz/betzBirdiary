...
# this function is also in sharedBird.py:
...
STATION_ID = "fbc09158-cfb3-46ed-b5fe-b2a77f2e2637" # "87bab185-7630-461c-85e6-c04cf5bab180"
STATION_API = "https://wiediversistmeingarten.org/api/station" # only for getting the station name, not for movements
...
def get_station_name(station_id):
    try:
        response = requests.get(STATION_API, timeout=30)
        ...
        # Build a simple mapping dictionary instantly
        station_map = {}
        for s in stations:
            name = s.get("name", "").strip()
            remote_id = s.get("station_id", "").strip()
            if name and remote_id:
                station_map[remote_id] = name
        
        # Look up directly using .get() with a default fallback value
        return station_map.get(station_id, "Unknown Station")
...
if __name__ == "__main__":
    ...
    html_output.append(f"<p>Human validations<br>of {station_name} ({STATION_ID})<br> from {earliest_date} to {current_day}</p>")
