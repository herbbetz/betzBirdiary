...
- rein mit `apt install python3-lgpio` ist lgpio auf Trixie veraltet und benötigt für `HX711.py` auch ein Downgrade auf alte Syntax der Aufrufe von `gpiochip`, siehe `show_lgpio_version.py` (lgpio 0.2.2 im Feb. 26, aktuelle Version benötigt Neuübersetzung oder `pip`). Das betrifft nicht den C-Daemon `hx711d.c`.
- Revision am 21.2.26:
