...
- Die Gewichts-Nullinie hat eine gewisse Variation (noise), die bei Wind und ebenso bei Feuchte (!) deutlich zunimmt. Dieser `baseline noise` kann durch Standardabweichung (Welford σ) ausgedrückt werden und auch zur Erhöhung der Auslöseschwelle verwendet werden, z.B.`dyn_weightThreshold = 2 * sigma + weightThreshold`.
