...
Außerdem kann das *Memory Layout* von .tflite Modellen differieren, besonders wenn sie aus anderen Modellen hergeleitet wurden (Tensorflow -> NHWC, PyTorch -> NCHW): 
````
layout = NHWC / NCHW
width
height
...
Unsere einfachen *label-basierten* Vogelmodelle können ihre implizit erfassten Informationen zum Vogel nicht semantisch ausgeben.  Tatsächlich gibt es auch eine systematischere KI-Klassifizierung von Vögeln, die auf die Erkennung ihrer *morphometrischen* Unterschiede (Schnabelform, Körperbau) trainiert ist. Die kalifornische Universität San Diego veröffentlichte dazu den Datensatz [CUB-200-2011](https://www.vision.caltech.edu/datasets/cub_200_2011/). Der Datensatz beinhaltet 200 nordamerikanische Vogelarten (keine europäischen Kohlmeisen = Great Tit). Seine Eigenschaften:  ~ 60 Bilder/Art = 11788 Bilder,  500x400 px. Auf jedem Bild ist der Vogel grob durch eine *bounding box* markiert, was einfach ist und Kontext (wie Wasser) beinhaltet. Außerdem ist der Vogel mit 15 Körperpunkten (*keypoints: "back-beak-belly-..."*) markiert (`part_locs.txt`) für ein grobes geometrisches Layout. Erst durch aufwändige Freistellung des Vogels vom Hintergrund (*segmentation masks*) sind dann seine Kontouren & Proportionen ausgebbar. Daneben sind insgesamt 312 Attributklassen definiert, z.B. für den Schnabel "has-bill-color: red-black-..." oder "has-bill-shape: curved-needle-cone-...". Diese können, falls so trainiert, zusätzlich zur erkannten Vogelart ausgegeben werden und dann wiederum für ein weiteres Vogelmodell auf Basis einer semantischen Eingabe (statt einer Bildeingabe) dienen.

Konkrete Vogelmodelle, die auf dem *CUB-200-2011* Datensatz beruhen, habe ich bisher nicht gefunden (auf Github, Huggingface). Der *CUB-200-2011* wird als wissenschaftliche Referenz auch nicht mehr erweitert. Dem Datensatz europäische Vogelarten hinzuzufügen, wäre ein Hobbyprojekt. Der hohe menschliche Kennzeichnungsaufwand (Masken, Keypoints, Attribute) lohnt sich aber nur wissenschaftlich (Modellerklärung, Vogelevolution). An derart ausführlicher Beschreibung von Spatzen ist kaum jemand interessiert.
