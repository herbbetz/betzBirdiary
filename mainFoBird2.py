...
import os
import subprocess # for message to mdroid, tasmota ...
import sys
import signal
...
def send_movement(wght):
    ...
    if (vid_cnt >= upmaxcnt):
        ...
        # only if system shutdown intended:
        cmd = "bash /home/pi/station2/tasmotaDown.sh limitdown"
        subprocess.call(cmd, shell=True) # returns exit code of cmd
        time.sleep(2)
        cleanAndExit()
        #
        return # never reached with program shutdown?
    ...
    # send circBuf + following {videodurate} secs video to vidData
    # guards against video size being unlimited
    circOut.start()
    time.sleep(videodurate) # wait for video to be filled, + 3 secs from CircularOutput
    circOut.stop()
    ...
    if upfail:
        ...
        cmd = "bash /home/pi/station2/mdroid.sh VideoUpfail_keepdir"
    else:
        vid_cnt += 1
        ms.setVidCnt(vid_cnt)
        cmd = "bash /home/pi/station2/mdroid.sh newVideo" + str(vid_cnt) # newVideo# + str(vid_cnt) would be interpreted by bash as comment after the # .
    subprocess.call(cmd, shell=True) # returns exit code of cmd
