...
from configBird2 import hxDataPin, hxClckPin, hxOffset, hxScale, weightThreshold, weightlimit # import variables from ./configBird.py
...
def judgeWeight(wght): # nested function, returns True if wght should be used for scaleAdjust
    global weightThreshold, weightlimit
    if (wght > weightlimit) or len(judgeWeight.vals) > 100: # often when balance strain gauge disconnected
        judgeWeight.vals = []
        ms.log(f"{wght} above {weightlimit}g OR over 100 cycles not plausible") #python f-string
        return False
