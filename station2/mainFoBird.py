...
def preloadCaptures(maxImg, sleep):
    for img in range(maxImg):
        ...
        camera.capture(imgName, resize=losize, use_video_port=True) # 3.4 of https://picamera.readthedocs.io/en/release-1.13/recipes1.html
        ms.setImgCnt(imgNum)
        time.sleep(sleep)
    return maxImg
...
if __name__ == "__main__": # must define process main for multiprocessing
    ...
    while True:
        try:
            ...
            camera.capture(imgName, resize=losize, use_video_port=True)
            ms.setImgCnt(imgNum)
